document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector("#formCreateSensor")?.addEventListener("submit", async e => {
    ...
    if (result.success) {
      ...
      scripts.showToast(result.message, "success"); // ✅ appel corrigé
    } else {
  ...
  <tr data-id="${sensor.id}">
    ...
    <td>
      <button class="btn btn-sm btn-link btn-edit" data-bs-toggle="modal" data-bs-target="#modalEdit"
        data-id="${sensor.id}"
        data-arduino_id="${sensor.arduino_id}"
        data-index_capteur="${sensor.index_capteur}"
        data-alias="${sensor.alias}"
        data-type="${sensor.type || ''}"
        data-adresse_mac="${sensor.adresse_mac || ''}">
...
function bindSensorActions() {
  document.querySelectorAll(".btn-edit").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      modal.querySelector("[name='type']").value = btn.dataset.type || "";
      modal.querySelector("[name='adresse_mac']").value = btn.dataset.adresse_mac || "";
    });
  });
